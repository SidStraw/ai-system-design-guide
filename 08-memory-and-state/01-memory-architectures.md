...
LLM 記憶體已從「history buffers」演進為**三層式認知架構**。這個階層結構模仿人類認知系統（L1-L3），以平衡速度、成本與回憶容量。現在的生產級 agent stack 傾向於在向量儲存之上，依賴專用的記憶層（Mem0、Zep、Letta、Cognee），而不是自行打造。
...
- **冗餘說明**：我們只在 L1 保留最近的幾輪對話與關鍵 system instructions。
...
L2 儲存本次 session 或與此使用者過去 session 中「先前發生了什麼」。
- **儲存**：向量資料庫（Pinecone、Weaviate、Qdrant）。
- **檢索**：語意搜尋。如果使用者問「上週二我們談了什麼？」，L2 會提供答案。
- **模式**：**Experience Replay**。Agent 會檢索過去成功的 trajectory，以引導當前決策。
...
- **真實性錨定**：當 L1 與 L2 提供互相衝突的資訊時，L3 會作為「Ground Truth」。
...
2. **注意力稀釋**：即使是「Long Context」模型，「Lost in the Middle」仍然存在。如果 context 被不相關的歷史對話塞滿，模型對*當前*任務的推理能力就會下降。
3. **延遲**：由於 KV cache 載入，TTFT（Time to First Token）會隨 context 大小增加而擴大。 
Staff-level 的架構會使用**策略式檢索（Strategic Retrieval）**，讓 context window 保持精簡且聚焦。

<a id="q-how-do-you-handle-privacy-leakage-in-tier-3-global-semantic-memory"></a>
### 問：你如何處理第 3 層（Global Semantic Memory）中的「Privacy Leakage」？
...
<a id="references"></a>
## 參考資料
- Pack 等人，"Generative Agents"（2023/2025 Context）
